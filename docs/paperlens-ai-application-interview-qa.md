...
可以。对彼此没有依赖的 text、figure、table 子任务，开启 `MULTI_AGENT_PARALLEL_ENABLED` 后可并发运行，再由 Reducer 合并结果。并行能够缩短跨模态任务的关键路径，但收益受 Provider 延迟、缓存和题型分布影响。当前 v4 Dev 的最终版本相对同一数据集修复前基线，平均延迟降低 18.93%、P95 降低 11.32%；这组变化还包含检索、路由和工具链修复，不能全部归因于并行本身。
...
### Q52：v4 Dev 36/36 是否等于系统准确率 100%？

不等于。它只表示在 5 篇论文、60 条 v4 Benchmark 中已经运行的 36 条 Dev 上，独立模型 Judge 判定 36 条任务成功。最新完整模型报告不是 v4 Test 报告，而且论文数量和题目规模仍然有限。正确表述是“在 v4 的 36 条 Dev 上取得 36/36 Task Success”；如果需要说明独立 Test，应另行引用早期 v2 冻结 Test 的 8/8 结果，不能把不同版本合并成一个准确率。
...
检索参数、Router 规则和提示词先在 Dev 上确定；Test 只按固定配置运行一次，不根据结果继续调参。当前 v4 的 24 条 Test 尚未运行，因此 README 只报告 v4 Dev，并把早期 v2 Test 明确标成历史对照。若修改数据标注或系统关键逻辑，应建立新版本数据集和报告，而不是覆盖旧结果制造不可比对照。
...
第一是按冻结配置完成 v4 Test，并增加人工抽检与 CI 质量门禁；第二是真正的身份认证、多租户记忆隔离和权限审计；第三是将进程内 SSE 事件迁移到 Redis Streams 或消息队列，支持多 Worker；第四是加入限流、成本预算、集中日志和告警。当前版本已能展示完整 AI 应用工程闭环，但不应声称已经达到大规模生产系统水平。

## 使用建议

面试时不要逐字背诵。建议每题先说结论，再结合一次真实链路解释取舍，最后主动说明边界。例如谈 SSE 时明确“节点与 Tool 事件实时、答案是校验后分片”；谈评测时明确“v4 共 60 条题，但最新完整结果来自 36 条 Dev，不代表生产准确率 100%”。这种表达比堆叠框架名称更能体现工程判断。
